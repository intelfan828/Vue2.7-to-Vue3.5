      ...
      <slick :options="slickOptions">
        <div v-for="(item, index) in safeTradeContent" :key="index">
          ...
          <div class="chart-info d-custom-flex align-items-center justify-space-between mb-4">
            ...
            <span class="primary--text font-md"
              ><i class="primary--text mr-2" :class="item.percentageIcon"></i>2.3%</span
            >
          </div>
          <span class="subheading"
            ><strong class="success--text mr-1">${{ item.market_cap }}</strong> Market
            Capitalisation</span
          >
        </div>
      </slick>
