...
const actions = {
    ...
    activeVoucher(context, page) {
        Nprogress.start();

        return new Promise((resolve, reject) => {
            JufrahAdminAPI.post(endpoints.ads+'/apply_voucher', {
                ad_id:page.ad_id,
                aduser_id:page.aduser_id,
                featured_token:page.featured_token,
            })
                .then((response) => {
                    context.dispatch('fetchAdReports')
                    context.commit('setFeatureAd', response.data)
                    resolve(response);
                })
                .catch((error) => {
                    Vue.notify({
    ...
    cancelVoucher(context, page) {
        Nprogress.start();

        return new Promise((resolve, reject) => {
            JufrahAdminAPI.post(endpoints.ads+'/remove_voucher', {
                ad_id:page.ad_id,
                aduser_id:page.aduser_id,
            })
                .then((response) => {
...
const mutations = {
    ...
    setFeatureAd(state, data) {
        state.selectedAd.featured = data.featured;
    },
