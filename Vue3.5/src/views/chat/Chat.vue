  ...
  <v-container fluid py-0 px-0>
    <template v-if="!loadingChatModule">
      ...
      <v-layout row wrap class="chat-layout px-3">
        <v-col xl2 lg3 md3 sm0 xs0 class="chat-sidebar">
          <chat-sidebar></chat-sidebar>
        </v-col>
        <v-col xl10 lg9 md9 sm12 xs12 class="chat-main">
          <chat-area></chat-area>
        </v-col>
      </v-layout>
    </template>
...
export default {
  ...
  mounted() {
    ...
    const defaultLayout = document.querySelector('.app-default-layout .v-main__wrap')
    const miniLayout = document.querySelector('.app-mini-layout .v-main__wrap')
  ...
  destroyed() {
    ...
    const miniLayout = document.querySelector('.app-mini-layout .v-main__wrap')
    const boxLayout = document.querySelector('.app-boxed-layout .app-content')
    const horizontallayout = document.querySelector('.app-horizontal-layout .v-main__wrap')
    if (defaultLayout) {
