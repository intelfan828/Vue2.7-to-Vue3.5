      ...
      <v-data-table :headers="headers" :items="data" hide-default-footer>
        <template v-slot:item="{ item }">
          <tr>
            ...
            <td>{{ item.shares }}</td>
            <td>
              <span class="v-badge primary white--text">{{ item.members }}</span>
            </td>
            <td :class="[{ 'success--text': item.status === 1, 'error--text': item.status === 0 }]">
              <i
                :class="[{ 'ti-arrow-up': item.status === 1, 'ti-arrow-down': item.status === 0 }]"
              ></i>
              {{ item.profit }}%
            </td>
          </tr>
