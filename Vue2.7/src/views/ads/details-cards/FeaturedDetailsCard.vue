  ...
  <div>
    <v-list class="card-list top-selling management-wrap">
      ...
      <v-list-item class="py-4">
        ...
        <v-list-item-action class="my-0">
          <h5 class="mb-0">{{ ad.is_featured && ad.featured.end_date ? `${timeDifference.days} Days
      ...
      <v-card class="d-flex justify-center mb-6 mt-4" style="gap: 4px" flat tile>
        <v-btn v-if="!ad.is_featured" :disabled="loading || !$can(this.CREATE, this.RESOURCE)" @click="applyVoucher()"
          depressed color="success" class="mx-0">
          {{ $t('Apply') }}
        </v-btn>
        <v-btn v-if="ad.is_featured" :disabled="loading || !$can(this.UPDATE, this.RESOURCE)" @click="applyVoucher()"
          depressed color="success" class="mx-0">
          {{ $t('Update') }}
        </v-btn>
        <v-text-field dense outlined class="name-input" v-model="code" required></v-text-field>
        <v-btn :disabled="loading || !ad.is_featured || !$can(this.DELETE, this.RESOURCE)" @click="openDeleteConfirmation"
          depressed color="error" class="mx-0">
          {{ $t('Cancel') }}
        </v-btn>
      </v-card>
      <delete-confirmation-dialog ref="deleteConfirmationDialog" heading="Cancel Confirmation"
        message="Are you sure you want to cancel this voucher?" @onConfirm="cancelVoucher">
...
export default {
  ...
  methods: {
    ...
    applyVoucher() {
      ...
      this.loading = true;
      let test = this.$store.dispatch('activeVoucher', params)
        .then((res) => {
          this.code = ''
          this.ad.is_featured = 1,
            console.log(res);
    ...
    cancelVoucher() {
      ...
      this.loading = true;
      let test = this.$store.dispatch('cancelVoucher', params)
        .then(() => {
