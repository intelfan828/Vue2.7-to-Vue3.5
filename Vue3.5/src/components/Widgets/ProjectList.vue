      ...
      <template v-slot:item="{ item }">
        <tr>
          ...
          <td class="text-left">
            <router-link
              class="black--text"
              :to="`/${getCurrentAppLayoutHandler() + '/crm/projectDetails/' + item.id}`"
          ...
          <td class="text-left text-nowrap">
            <img
              ...
              width="25"
              height="25"
              v-for="(img, index) in item.teamImage"
              :key="index"
              :src="img"
            />
          </td>
          <td class="text-left">
            <v-chip v-if="item.status == 'Active'" color="primary" text-color="white">{{
              item.status
            }}</v-chip>
