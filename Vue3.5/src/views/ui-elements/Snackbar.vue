<template>
  <div class="custom-flex">
    ...
    <v-container fluid grid-list-xl pt-0>
      <app-card :heading="$t('message.snackbar')">
        <div class="mb-4">
          <p>
            The standard snackbar is useful for calling attention to some function that has just
            happened.
          </p>
        </div>
        <v-layout row wrap>
          ...
          <v-col xs12 md6>
            ...
            <v-btn block color="primary" large @click.native="snackbar = true">{{
              $t('message.showSnackbar')
            }}</v-btn>
          </v-col>
        </v-layout>
        <v-snackbar
      ...
      <app-card :heading="$t('message.contextualSnackbar')">
        <div class="mb-4">
          <p class="mb-0">
        ...
        <v-layout row wrap>
          ...
          <v-col xs12 md6>
            <v-text-field v-model="text" type="text" label="Text"></v-text-field>
            <v-text-field v-model.number="timeout" type="number" label="Timeout"></v-text-field>
            <v-btn block large color="warning" @click.native="snackbar2 = true" dark>{{
              $t('message.showSnackbar')
            }}</v-btn>
          </v-col>
        </v-layout>
