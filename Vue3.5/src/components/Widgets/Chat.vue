<template>
  ...
  >
    <div class="chat-widget">
      <PerfectScrollbar id="chat-scroll" style="height: 375px" :settings="settings">
        <div class="chat-body">
          <template v-for="(chat, index) in chats" :key="index">
            <div class="chat-block mb-2" :class="{ 'flex-row-reverse': chat.isAdmin }">
              <template v-if="chat.isAdmin">
                <img
                  ...
                  width="40"
                  height="40"
                  class="rounded-circle ml-3 mt-3"
                />
                <div class="chat-bubble-wrap">
                  <span class="fs-12 fw-normal grey--text text-right d-block mb-1">{{
                    chat.adminName
                  }}</span>
    ...
    <div class="send-message-box" slot="footer">
      <div class="d-custom-flex">
        ...
        <v-btn dark class="ml-3 px-5" large color="primary" @click="sendMessage"> Send </v-btn>
      </div>
    </div>
  </app-card>
</template>
...
export default {
  ...
  methods: {
    ...
    getReply() {
      let randomMessage = Math.floor(Math.random() * this.randomMessages.length)
      let reply = {
        id: new Date().getTime(),
