...
const actions = {
  ...
  createOptionValue(context, value) {
    ...
    const url = endpoints.createOptionValue + state.selectedOption.id + '/values'
    const optionValue = {
      values: [value],
    }
    ...
    return new Promise((resolve, reject) => {
      JufrahAdminAPI.post(url, optionValue)
        .then((response) => {
          ...
          context.dispatch('fetchSelectedCategory', state.selectedCategory.category).then(() => {
            Vue.set(
              state,
              'selectedOption',
  ...
  updateOptionValueOrder({ state }, payload) {
    Nprogress.start()
    console.log(payload)
    const url = endpoints.createOptionValue + state.selectedOption.id + '/sortValues'
    return new Promise((resolve, reject) => {
  ...
  createCategoryOption(context, categoryOption) {
    ...
    const url = endpoints.createCategoryOption + state.selectedCategory.category.id + '/options'
      ...
      JufrahAdminAPI.post(url, options)
        ...
        .catch((error) => {
          ...
          VueNotify({
            group: 'loggedIn',
            type: 'error',
            text: error.response.data.error ? error.response.data.error : 'Error Occurred',
          })
          reject(error)
        })
        .finally(() => {
  ...
  updateCategoryPayment(context, option) {
    ...
    const url = endpoints.updateCategoryPayment + state.selectedCategory.category.id + '/payment'
...
const mutations = {
  ...
  setSelectedOption(state, option) {
    ...
    if (option.hasOwnProperty('values')) state.selectedOptionValues = option.values
